...
- Choose skill execution architecture before implementing new or changing repeated workflows.
...
Before creating, adapting, composing, or hardening a skill, crow.pet should choose the smallest useful execution shape.

Supported v0.1 shapes are `inline`, `thin-references`, `deterministic-script`, `composed-pipeline`, and `manual-side-effect`.

Use `inline` for short repeatable guidance.

Use `thin-references` when the root skill should stay small but needs larger support material.

Use `deterministic-script` when repeated validation or transformation can be done outside the model context.

Use `composed-pipeline` only when multiple stages, noisy inputs, or high-output work benefit from isolation and compact stage handoffs.

Use `manual-side-effect` when the workflow publishes, deploys, spends, messages externally, or mutates high-risk systems.

Skill architecture plans must define isolation, handoff strategy, dynamic context, context budget, cleanup policy, capability needs, and verification.

Runtime-specific features must be mapped by adapters.

Canonical skills must not assume one harness's fork, subagent, shell injection, tool allowlist, or browser-control mechanism.
